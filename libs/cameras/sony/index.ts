...
const findSupportedNetwork = (networks) => {
  ...
  networks.forEach((network) => {
    if (
      !!_.find(SUPPORTED_CAMERA_NETWORKS, (supportedNetwork) =>
        _.includes(network.ssid, supportedNetwork)
      )
    ) {
      found = network;
    }
...
export class SonyCamera implements CameraInterface {
  ...
  private async connectWifi() {
    this.clientProxy.sendStatus(
      'PAGES.SETUP.FOTOBOX.MODAL.STATUS_CONNECTING_CAMERA'
    );
    ...
    do {
      ...
      logger.info(
        `Available networks: ${_.map(networks, (network) => network.ssid)}`
      );
    ...
    if (
      ...
      const { success: connectSuccess, msg: connectMsg } = await promisify(
        wifi.connectToAP
      )({
  ...
  private findCamera(): Promise<string> {
    this.clientProxy.sendStatus(
      'PAGES.SETUP.FOTOBOX.MODAL.STATUS_SEARCHING_CAMERA'
    );
